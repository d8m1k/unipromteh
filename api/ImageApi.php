...
class ImageApi{
     ...
     */
    function resize($source_file, $width, $height)    {
       $original_file = $source_file;
        ...
        $originals_dir = 'img/';
        $preview_dir = 'img/_preview/';
        if (!file_exists($originals_dir . $original_file))
            return 'Нет файла: '.$originals_dir . $original_file;


        $preview_path = $preview_dir . $resized_file;
        if (!file_exists($preview_path )) {

            if (!file_exists(dirname($preview_path)))
                mkdir(dirname($preview_path), 0700, true);


            if (class_exists('Imagick'))
                $this->image_constrain_imagick($originals_dir . $original_file, $preview_path, $width, $height);
            else
                $this->image_constrain_gd($originals_dir . $original_file, $preview_path, $width, $height);
        }
    ...
    public function add_resize_params($filename, $width=0, $height=0, $set_watermark=false)
    {
        ...
        if($width>0 || $height>0)
            $resized_filename = $file.'-'.$height.'.'.$ext;
